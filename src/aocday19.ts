function day19() {
  ...
  const [rules, messages] = puzzle.split('\n  \n')

  const ruleMap = new Map<number, string>();
  rules.split('\n').forEach(s => {
    const [index, rule] = s.trim().split(':')


    ruleMap.set(+index, rule.trim().replace(/\"/g, ''))
  });
  const messageArr = messages.split('\n').map(s => s.trim());

  const answer1 = (function () {
    let ruleZero = ruleMap.get(0);

    while(/\d/.test(ruleZero)) {
      ruleZero = ruleZero.replace(/\d{1,3}/g, (match) => {
        const rule = ruleMap.get(+match);
        return rule.includes('|') ? `(${rule})` : rule;
      })

    }

    const regex = new RegExp(`^${ruleZero}$`.replace(/\s/g, ''));
    // console.log(regex)

    return messageArr.filter(message => regex.test(message)).length;
  }())
    ...
    while(/\d/.test(ruleZero)) {
      ruleZero = ruleZero.replace(/\d{1,3}/g, (match) => {
        ...
        switch(+match) {
          ...
          case 11:
            // 正規太爛 直接暴力法 YO
            rule = '42 (42 (42 (42 (42 (42 (42 31)* 31)* 31)* 31)* 31)* 31)* 31'
            break;
        }
