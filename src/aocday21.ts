function day21() {
  ...
  const puzzle2 = `mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
  ...
  const answer1 = (function () {
    const allergenMap = new Map<string, [number, Map<string, number>]>();
    const ingredientCountMap = new Map<string, number>();
  
    puzzleArr.forEach(line => {
      const [_, ingredients, __, allergens] = line.match(/(.*?)\((contains (.*?))\)/);

      const ingredientArr = ingredients.trim().split(' ');
      const allergenArr = allergens.split(',').map(s => s.trim());
      ...
      allergenArr.forEach(allergen => {
        const [count, ingredientMap] = allergenMap.get(allergen) || [0, new Map()];
        ingredientArr.forEach(ingredient => {
          ingredientMap.set(ingredient, (ingredientMap.get(ingredient) || 0) + 1)
        })
        // [ 統計過敏原出現在食譜中的次數, 過敏原可能的材料以及出現的次數 ]
        allergenMap.set(allergen, [count+1, ingredientMap])
      })
    })
    ...
    while(mapping.size !== oldSize) {
      // 直到所有過敏源都確認為止
      oldSize = mapping.size;

      allergenMap.forEach(([allergenCount, map], allergen) => {

        let count = 0;
        let target = '';
        map.forEach((ingredientCount, ingredient) => {
          if (ingredientCount === allergenCount && !mapping.has(allergen) && !done.includes(ingredient)) {
            // 尋找材料出現次數與過敏原出現次數相同
            count++;
            target = ingredient;
          }
      ...
      allergenMap.forEach(([allergenCount, map], allergen) => {
        // 從食譜中刪除已經確定會過敏的材料
        mapping.forEach((ingredient) => {
          if (map.has(ingredient)) {
            map.delete(ingredient);
            allergenMap.set(allergen, [allergenCount, map]);
          }
        })
      })
    }
    ...
    // 尋找可能不含過敏源的材料
    const possiblySafe = new Set<string>();
    allergenMap.forEach(([num, map], allergen) => {
      map.forEach((_, ingredient) => {
        possiblySafe.add(ingredient);
      })
    })
    console.log(mapping, possiblySafe);

    let sum = 0;
    possiblySafe.forEach((ingredient) => {
      sum += ingredientCountMap.get(ingredient)
    })
    return { mapping, sum };
  }())
  ...
  const answer2 = (function () {
    ...
    return allergenList.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())).map(allergen => answer1.mapping.get(allergen)).join(',');
  }())
