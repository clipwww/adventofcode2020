function day4() {
  const puzzle = `byr:2024 iyr:2016
...
byr:1959`
  ...
  // part 1
  const answer1 = puzzleArr.filter(passport => requireFields.every(field => passport.includes(field))).length;
  console.log('part1', answer1);


  // part 2
  const answer2 = puzzleArr
    .filter(passport => requireFields.every(field => passport.includes(field)))
    .map(passport => passport.replace(/\n/g, ' ').split(' '))
    .filter(fields => {
      // const arr = passport.match(/byr:(192[0-9]|19[3-9]\d|200[0-2]) iyr:\d{4}/);
      return fields.every(field => {
        ...
        switch (key) {
          ...
          case 'hgt':
            const [_, num, unit] = value.match(/(\d{1,})(\w{1,})/);
            switch (unit) {
              case 'cm':
          ...
          case 'ecl':
            return /\b(amb|blu|brn|gry|grn|hzl|oth)\b/.test(value);
