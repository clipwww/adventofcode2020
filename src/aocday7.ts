...
striped crimson bags contain 5 dotted black bags.`
  ...
  const puzzle2 = `shiny gold bags contain 2 dark red bags.
...
dark violet bags contain no other bags.`
  ...
  // part 1
  function getRules(targetColor: string, rules: string[] = []) {
    let newRules = [];

    puzzleArr
      .forEach(bagStr => {
        if (bagStr.indexOf(targetColor) > 0) {
          const arr = bagStr.match(/(\w+\s\w+) bags contain/)
          if (arr && !rules.includes(arr[1])) {
            newRules.push(arr[1])
          }
        }
      })
    ...
    [
      ...newRules,
      ...newRules.map(color => getRules(color, [...newRules, ...rules])).reduce((pre, cur) => [...pre, ...cur], [])
    ].forEach(color => set.add(color))

    return Array.from(set);
  }

  const answer1 = getRules('shiny gold').length;
  console.log('part1', answer1);


  // part 2
  function getBags(target: string) {
    const main = puzzleArr.find(bagStr => bagStr.indexOf(target) === 0)
    const arr = main.match(/(\d)\s(\w+\s\w+)\s(bag|bags)/g)
    ...
    const colors = [];
    arr.map(str => str.replace(/(bag|bags)/g, '').trim()).forEach(str => {
      const [count] = str.match(/\d/g);

      colors.push(...Array(+count).fill('').map(() => str.replace(/\d/, '').trim()));
    })

    return [
      ...colors,
      ...colors.map(color => getBags(color)).reduce((pre, cur) => [...pre, ...cur], [])
    ]
  }
